...
{
	count = 0;
	params = load_data("bwapi-data\\AI\\4c.model");
}
...
template <typename T>
vector<size_t> PredictModel::sort_indexes(const vector<T> &v) {
...
vector<size_t> PredictModel::getPredictUnits(vector<double> input_m)
{
	vector<int> input_s;
	input_s.push_back(1);
	...
	return sort_indexes(result.second);
}