...
StrategyManager::StrategyManager()
	...
	, _predModel(PredictModel::Instance())
{
}
...
const std::vector<int> StrategyManager::getBinaryFeature(double value, std::vector<int> interval)
{
	int length = interval.size();
	std::vector<int> result;
	bool found = false;
	for (auto it = interval.begin(); it != interval.end(); )
	{
		if (found)
		{
			result.push_back(0);
...
const MetaPairVector StrategyManager::getMLPBuildOrderGoal()
{
	// the goal to return
	MetaPairVector goal;
	...
	UAB_ASSERT(features.size() == 59, "Extract Resource Features error");
	...
	features.push_back(BWAPI::Broodwar->self()->getUpgradeLevel(BWAPI::UpgradeTypes::Protoss_Ground_Armor));
	features.push_back(BWAPI::Broodwar->self()->getUpgradeLevel(BWAPI::UpgradeTypes::Protoss_Air_Armor));
	features.push_back(BWAPI::Broodwar->self()->getUpgradeLevel(BWAPI::UpgradeTypes::Protoss_Ground_Weapons));
	features.push_back(BWAPI::Broodwar->self()->getUpgradeLevel(BWAPI::UpgradeTypes::Protoss_Air_Weapons));
	features.push_back(BWAPI::Broodwar->self()->getUpgradeLevel(BWAPI::UpgradeTypes::Singularity_Charge));
	features.push_back(BWAPI::Broodwar->self()->getUpgradeLevel(BWAPI::UpgradeTypes::Leg_Enhancements));
	features.push_back(BWAPI::Broodwar->self()->getUpgradeLevel(BWAPI::UpgradeTypes::Protoss_Plasma_Shields));
	features.push_back(BWAPI::Broodwar->self()->getUpgradeLevel(BWAPI::UpgradeTypes::Carrier_Capacity));

	UAB_ASSERT(features.size() == 89, "Extract Resource Features error");
	...
	UAB_ASSERT(features.size() == 122, "Extract unit Features error");
	//BWAPI::Broodwar->printf("features length: %d", features.size());
	vector<size_t> predictUnits = _predModel.getPredictUnits(features);
	_predModel.count++;
	int k = 0;
	int topNum = 3;
	...
	while (k < topNum) {
		k++;
		int predictUnit = predictUnits[k];
		...
		case 4:
			...
			break;
		case 5:
			goal.push_back(MetaPair(BWAPI::UnitTypes::Protoss_Dark_Templar, 2));
			break;
		case 6:
			goal.push_back(MetaPair(BWAPI::UnitTypes::Protoss_Dragoon, 5));
			break;
		case 7:
			if (numProbe < maxProbes)
			{
				goal.push_back(MetaPair(BWAPI::UnitTypes::Protoss_Probe, 1));
			}
			else
			{
				if (topNum < predictUnits.size())
				{
					topNum++;
				}
				continue;
			...
			break;
		case 8:
			goal.push_back(MetaPair(BWAPI::UnitTypes::Protoss_Reaver, 1));
			break;
		case 9:
			goal.push_back(MetaPair(BWAPI::UnitTypes::Protoss_Zealot, 5));
			break;
		case 10:
		...
		case 14:
			...
			break;
		case 15:
			if ((UnitUtil::GetAllUnitCount(BWAPI::UnitTypes::Protoss_Cybernetics_Core) > 0) ||
		...
		case 19:
			...
			break;
		case 20:
			if ((UnitUtil::GetAllUnitCount(BWAPI::UnitTypes::Protoss_Robotics_Support_Bay) > 0) ||
			...
			break;
		case 21:
			if ((UnitUtil::GetAllUnitCount(BWAPI::UnitTypes::Protoss_Fleet_Beacon) > 0) ||
		...
		case 23:
			...
			break;
		case 24:
			upgrade = BWAPI::UpgradeTypes::Protoss_Air_Weapons;
			currentLevel = self->getUpgradeLevel(upgrade);
			if (self->isUpgrading(upgrade) || (currentLevel == 3))
			{
				if (topNum < predictUnits.size())
				{
					topNum++;
			...
			{
				goal.push_back(MetaPair(upgrade, currentLevel + 1));
			}
			break;
		case 25:
			upgrade = BWAPI::UpgradeTypes::Protoss_Air_Armor;
			currentLevel = self->getUpgradeLevel(upgrade);
			if (self->isUpgrading(upgrade) || (currentLevel == 3))
			{
				if (topNum < predictUnits.size())
		...
		case 28:
			...
			break;
		case 29:
			upgrade = BWAPI::UpgradeTypes::Singularity_Charge;
			currentLevel = self->getUpgradeLevel(upgrade);
			if (self->isUpgrading(upgrade) || (currentLevel == 1))
			{
				if (topNum < predictUnits.size())
				{
					topNum++;
			...
			break;
		case 30:
			upgrade = BWAPI::UpgradeTypes::Leg_Enhancements;
			currentLevel = self->getUpgradeLevel(upgrade);
			if (self->isUpgrading(upgrade) || (currentLevel == 1))
			{
				if (topNum < predictUnits.size())
			...
			break;
		case 31:
			upgrade = BWAPI::UpgradeTypes::Carrier_Capacity;
			currentLevel = self->getUpgradeLevel(upgrade);
			if (self->isUpgrading(upgrade) || (currentLevel == 1))
			{
				if (topNum < predictUnits.size())
...
const MetaPairVector StrategyManager::getProtossBuildOrderGoal()
{
	// the goal to return
	_predModel.count++;
	BWAPI::Broodwar->printf("predict times: %d", _predModel.count);
	MetaPairVector goal;
